...
class Network:
    ...
    def build(self, args, num_words, num_chars):
        ...
        embedded = Dense(args.attention_dim, activation=None)(embedded)
        ...
        self.model = tf.keras.Model(inputs=[word_ids, charseq_ids, charseqs, positional_encoding], outputs=predictions)
    ...
    @tf.function(input_signature=[[tf.TensorSpec(shape=[None, None], dtype=tf.int32)] * 3 + [tf.TensorSpec(shape=[None, None, None], dtype=tf.float32)], [tf.TensorSpec(shape=[None, None], dtype=tf.int32)]*MorphoDataset.TAGS])
    ...
    def train_epoch(self, epoch, dataset, log_file, args):
        ...
        for i, batch in enumerate(dataset.batches(args.batch_size)):
            self.train_batch(
                [batch[dataset.FORMS].word_ids, batch[dataset.FORMS].charseq_ids, batch[dataset.FORMS].charseqs, batch[-1]],
...
if __name__ == "__main__":
    ...
    args.directory = f"{args.directory}/models/positional_{architecture}"
