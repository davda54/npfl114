...
class Caltech42:
    ...
    class CrossvalidationFold:
        def __init__(self, positive_class, train_data, dev_data, preprocessing, augmentation):
            self.train = Caltech42.Dataset(positive_class, train_data, augmentation, shuffle_batches=True)
            self.dev = Caltech42.Dataset(positive_class, dev_data, preprocessing, shuffle_batches=False)


    class Dataset:
        def __init__(self, positive_class, data, transformation, shuffle_batches, seed=42):
            self._data = data
            self._size = len(self._data["images"])

            self._shuffler = np.random.RandomState(seed) if shuffle_batches else None
            self._transformation = transformation
            self._data["labels"] = (np.array(self._data["labels"]) == positive_class).astype(int)
...
#                                     (Caltech42.MIN_SIZE, Caltech42.MIN_SIZE, Caltech42.C), np.float32))
    ...
    def __init__(self, positive_class, augmentation, preprocessing, folds):
        """
        Parameters
        ----------
        ...
        self.test = self.Dataset(self.labels.index(positive_class), self.test, preprocessing, shuffle_batches=False)
        ...
        for subset in subsets:
            self.folds.append(self.CrossvalidationFold(self.labels.index(positive_class), subset['train'], subset['dev'], preprocessing, augmentation))
